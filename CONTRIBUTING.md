...
기술 분석 글은 가능한 경우 독자가 로컬에서 직접 실행할 수 있는 재현 실험을 포함합니다.

- 외부 시스템을 공격하지 않는 로컬 시뮬레이션 또는 격리된 예제를 사용합니다.
- 실험 목표, 준비물, 실행 명령, 입력 조건, 예상 결과와 해석을 함께 적습니다.
- 취약한 구성과 방어 구성을 같은 입력으로 비교합니다.
- 공격 성공률뿐 아니라 정상 작업 성공률과 오탐도 함께 측정합니다.
- 실제 자격증명, 운영 주소, 악성 페이로드와 무단 접근 절차는 포함하지 않습니다.
- 실험 코드는 `labs/`에 두고 자동 테스트로 예상 결과를 고정합니다.

## 체크리스트 (PR 전)

- [ ] `AICRA 공개 연구 제안` 또는 `공개 연구 제안` 절을 넣지 않았는가?
- [ ] 재현 실험이 있다면 안전 범위, 실행 명령, 예상 결과와 테스트가 포함됐는가?
- [ ] 제목이 명확한가?
- [ ] description이 작성되었는가?
- [ ] 카테고리와 태그가 적절한가?
