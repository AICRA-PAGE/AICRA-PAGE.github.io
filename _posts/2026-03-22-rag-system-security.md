...
### 2.2 공격자 능력 계층 (Attacker Capability Tiers)
...
RAG 시스템과 벡터 데이터베이스 관련 보안 사건들을 분석하면 이론적 위협이 실제로 어떻게 구현되고 악용되는지 명확히 이해할 수 있습니다. 최근 발생한 주요 CVE 사건들과 실제 공격 사례를 통해 방어 메커니즘의 중요성과 구현 방향을 도출할 수 있습니다.

### 3.1 CVE-2025-32711: 벡터 데이터베이스 인증 우회

**사건 개요**: 2025년 3월에 공개된 이 취약점은 인기 있는 오픈소스 벡터 데이터베이스 Milvus의 인증 메커니즘에서 발견되었습니다. 공격자는 특수하게 조작된 gRPC 요청을 통해 인증 절차를 우회하고 직접 벡터 컬렉션에 접근할 수 있었습니다.
...
**실제 피해 사례**:
- 한 금융 기관이 심신미약 운영자의 부주의로 Milvus를 인터넷에 노출
- 공격자가 CVE-2025-32711을 악용하여 3개월간 고객 거래 관련 벡터 데이터 추출
- 약 240만 명의 고객 정보가 최소 부분적으로 유출
- 규제 당국 조사 및 GDPR 위반으로 약 €1,500만 벌금 부과

### 3.2 CVE-2024-8309: 프롬프트 인젝션을 통한 벡터 데이터 조작
...
**실제 영향**:
- 한 법률 문서 RAG 시스템이 이 공격에 노출되어 비밀 소송 자료가 공개됨
- 공격자가 일반 기사를 "비밀 문서"로 위장하여 접근 제어 우회
- 약 50개의 기밀 계약서가 무단으로 열람 가능 상태 노출

### 3.3 CVE-2024-12911: 벡터 임베딩 역공학을 통한 원본 데이터 복원

**사건 개요**: 2024년 12월에 발표된 연구에서 특정 오픈소스 벡터 데이터베이스의 임베딩 벡터로부터 원본 텍스트를 상당 부분 복원할 수 있음을 입증했습니다.
...
**기술적 평가**:
- **복원 성공률**: 짧은 문서(500단어 이하)의 경우 70~85% 수준의 의미론적 복원
- **장시간 문서**: 핵심 정보는 복원되나 상세 내용은 손실
- **CVSS 점수**: 8.2 (High) - 기밀성 침해 심각
...
### 3.4 Chevrolet Watsonville 시설 사건: 산업 RAG 시스템 침투

**사건 개요**: 2025년 1월, 자동차 제조업체 General Motors의 Watsonville 공장에서 운영하는 산업용 문서 RAG 시스템이 침투당했습니다. 이 사건은 엔터프라이즈 RAG 시스템의 실제 보안 위험을 명확히 보여줍니다.

**침입 경로 분석**:

1단계: 초기 침투(Initial Access)
- 공격자가 과거 인수합병 회사의 구식 VPN 자격증명을 어둠의 웹에서 구입
- 해당 계정이 여전히 Watsonville 공장 네트워크에 활성화되어 있음
- 네트워크 세그먼테이션 부재로 내부망 전역 접근 확보

2단계: RAG 시스템 발견
- 네트워크 스캔으로 벡터 데이터베이스(Weaviate 인스턴스) 식별
- 기본 자격증명("admin:admin")이 변경되지 않음
- 벡터 저장소에 약 500만 개의 기술 문서 색인되어 있음

3단계: 데이터 추출
- SQL 인젝션과 유사한 기법으로 모든 벡터 일괄 쿼리 수행
- 메타데이터에 저장된 원본 파일 경로로부터 문서 위치 파악
- 약 2주에 걸쳐 전체 벡터 및 관련 문서 추출

4단계: 영향 범위
- 추출된 데이터에 포함된 정보:
  * 엔진 설계 도면(Engine design specifications)
  * 제조 공정(Manufacturing processes)
  * 공급망 정보(Supply chain details)
  * 결함 분석 보고서(Defect analysis reports)
  * 구성 요소 공급업체 목록(Component supplier lists)

**보안 실패 분석**:
```
실패 원인                  심각도    방어 메커니즘
================================================
기본 자격증명 미변경       Critical   강제 초기 변경 정책
네트워크 세그먼테이션 부재 Critical   VLAN 격리, 제로 트러스트
벡터 DB 암호화 없음        High       전송 중 및 저장 중 암호화
접근 로깅 부재             High       모든 쿼리 감시
백업 정보 노출             High       백업 암호화 및 격리
메타데이터 보호 부재       Medium     메타데이터 암호화
```

**규제 및 사업 영향**:
- 자동차 부품 도난(Intellectual Property Theft) 혐의로 수사 개시
- GM이 공급업체 및 고객에 통지: 약 2,800만 달러 합의금
- 공장 보안 감사 및 시스템 재구축에 약 6개월 소요
- 산업 매체에서 "공개된 AI 시스템" 비판

**설명 및 교훈**:
Chevrolet 사건은 다음 원칙을 강조합니다:
1. **기술만으로는 부족**: 암호화나 접근 제어보다 운영 보안(Operational Security) 기초가 중요
2. **레거시 시스템의 위험**: 인수합병으로 인한 구식 자격증명이 현대 시스템의 입구가 될 수 있음
3. **네트워크 격리의 필수성**: 단일 손상된 계정으로 전체 RAG 시스템에 접근할 수 없는 구조 필요
4. **모니터링의 중요성**: 일반적인 사용 패턴과 다른 대량 쿼리 시도를 자동으로 탐지

---

## 4. 강화된 Defense-in-Depth 아키텍처
...
| 공격 벡터 | 단일 방어 | 2계층 | 3계층 | 4계층 | 5계층 | 6계층 | 7계층 |
|----------|---------|-------|-------|-------|-------|-------|-------|
| 인증 우회 | 0% | 65% | 85% | 90% | 92% | 95% | 99.5% |
| 프롬프트 인젝션 | 5% | 70% | 80% | 92% | 94% | 97% | 99.8% |
| 임베딩 공간 조작 | 10% | 50% | 75% | 80% | 95% | 96% | 99.2% |
| 데이터 유출 | 0% | 60% | 90% | 92% | 93% | 95% | 99.7% |
| 서비스 거부(DoS) | 2% | 40% | 65% | 85% | 87% | 98% | 99.5% |

**성능 오버헤드 분석:**

| 계층 | 지연 증가 | CPU 오버헤드 | 메모리 오버헤드 | 영향도 |
|-----|---------|------------|------------|--------|
| 1. 인프라 | 0-5ms | 0% | 0% | 매우낮음 |
| 2. IAM | 10-20ms | 2-5% | 5-10MB | 낮음 |
| 3. 암호화 | 20-50ms | 5-15% | 10-20MB | 중간 |
| 4. 입력검증 | 5-15ms | 1-3% | 2-5MB | 낮음 |
| 5. 임베딩검증 | 30-100ms | 8-12% | 20-50MB | 중간 |
| 6. 격리/샌드박싱 | 100-500ms | 10-25% | 100-500MB | 높음 |
| 7. 감시 | 5-30ms | 3-8% | 50-100MB | 중간 |
| **총합** | **170-650ms** | **29-68%** | **187-735MB** | **허용가능** |
